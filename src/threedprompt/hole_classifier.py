...
File: /home/user/3d-printing-model-prompt/src/threedprompt/hole_classifier.py
Description: Pure-Python heuristics that decide, for each boundary-edge
    ...
    on bpy so it can be unit tested without Blender installed. Named
    hole_classifier (not classifier.py) to avoid colliding with the
    existing prompt-complexity classifier.py in this package - the two
    are unrelated.
Inputs: A WatertightReport (see models.py) whose holes still carry the
    default AMBIGUOUS classification from watertight.py's Blender pass.
...
Troubleshooting:
    ...
    - If tiny stray-vertex gaps stop getting flagged, check that
      blender_scripts/analyze_watertight.py is still reporting true
      per-vertex loop area/perimeter and not a degenerate 0-area loop
      (which sorts as "ambiguous" here rather than "defect" - see
      classify_hole below).
"""
...
def _position_score(hole: Hole, bbox: BoundingBox) -> float:
    ...
    (open top of a cup, open underside of a stand).
    """
    height = bbox.size[2]
    if height <= 0:
        return 0.0
    ...
    # model), never up to 1.0, so normalize the falloff against 0.5 - not
    # 1.0 - or a hole at the true middle would still score ~0.5 instead of
    # the intended ~0.
...
def classify_hole(hole: Hole, bbox: BoundingBox, total_surface_area: float) -> Hole:
    """Score one hole and return a copy with classification/confidence/reason filled in. Does not mutate the input."""
...
def classify_report(report: WatertightReport) -> WatertightReport:
    """Return a copy of report with every hole's classification/confidence/reason filled in by classify_hole."""
    updated = deepcopy(report)
    updated.holes = [classify_hole(hole, report.bounding_box, report.total_surface_area) for hole in report.holes]
    return updated
