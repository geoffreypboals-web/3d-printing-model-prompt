...
| POST   | `/generate`                   | `{"prompt": "...", "wall_thickness_mm": 3.0}` -> model    |
| GET    | `/models/{model_id}/download` | Download the STL                                          |
| POST   | `/models/{model_id}/thicken`  | `{"amount_mm": 1.5}` -> thickened model (see above)        |
| POST   | `/thicken`                    | multipart upload (`file`, `amount_mm`) -> thickened model  |
